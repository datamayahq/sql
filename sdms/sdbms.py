...
class DBHelper():
    ...
    def exportToCSV(self, filename='student_records.csv'):
        try:
            ...
            with open(filename, mode='w', newline='') as file:
                ...
                # Write header
                writer.writerow(['Roll', 'Name', 'Department', 'Year', 'Course A', 'Course B', 'Course C'])
                # Write data
                writer.writerows(data)

            QMessageBox.information(QMessageBox(), 'Export Successful', f'Records exported to {filename} successfully.')
        except Exception as e:
            QMessageBox.warning(QMessageBox(), 'Error', f'Failed to export records: {str(e)}')

    def exportToExcel(self, filename='student_records.xlsx'):
        try:
            self.c.execute("SELECT * FROM student")
            data = self.c.fetchall()
            ...
            ws = wb.active
            # Write header
            ws.append(['Roll', 'Name', 'Department', 'Year', 'Course A', 'Course B', 'Course C'])
            # Write data
            for row in data:
                ws.append(row)
...
class Window(QMainWindow):
    def __init__(self):
        ...
        # New button for exporting records
        self.btnExport = QPushButton("Export", self)
        self.btnExport.move(205, 220)
        self.btnExport.resize(180, 40)
        self.btnExportFont = self.btnEnterStudent.font()
        self.btnExportFont.setPointSize(13)
        self.btnExport.setFont(self.btnExportFont)
        self.btnExport.clicked.connect(self.exportRecords)
        layout.addWidget(self.btnExport, 2, 1)
    ...
    def exportRecords(self):
        ...
        file_name, _ = QFileDialog.getSaveFileName(self, "Save Records", "", "CSV Files (*.csv);;Excel Files (*.xlsx);;All Files (*)", options=options)

        if file_name:
            if file_name.endswith('.csv'):
                self.exportToCSV(file_name)
            elif file_name.endswith('.xlsx'):
                self.exportToExcel(file_name)
            else:
                QMessageBox.warning(QMessageBox(), 'Invalid File Format', 'Please choose a valid CSV or Excel file format for export.')

    def exportToCSV(self, filename):
        db_helper = DBHelper()
        db_helper.exportToCSV(filename)

    def exportToExcel(self, filename):
        db_helper = DBHelper()
        db_helper.exportToExcel(filename)
